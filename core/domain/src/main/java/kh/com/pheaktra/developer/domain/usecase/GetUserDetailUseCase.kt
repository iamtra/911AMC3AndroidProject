...
) : BaseUseCase<Int, Flow<BaseUiState<UserModelResponse>>>() {
    override suspend fun execute(params: Int): Flow<BaseUiState<UserModelResponse>> {
        return flow {
            try {
                val response = userRepository.getUserDetail(params)
                if (response.result) {
