package kh.com.pheaktra.developer.basic.advance.android.weekend.data.storage

import kh.com.pheaktra.developer.model.general.MaterialComponentModel
import kh.com.pheaktra.developer.basic.advance.android.weekend.navigation.NavKey

val componentList = listOf(
    MaterialComponentModel(
        1,
        "Badge",
        "A small circle that can contain a number or icon",
        NavKey::Badge,
        "https://img.icons8.com/color/96/badge.png",
    ),
    MaterialComponentModel(
        2,
        "Bottom Sheet",
        "A sheet that slides up from the bottom of the screen",
        NavKey::BottomSheet,
        "https://img.icons8.com/color/96/menu.png",
    ),
    MaterialComponentModel(
        3,
        "Buttons",
        "Different types of buttons: Filled, Tonal, Outlined, etc.",
        NavKey::Buttons,
        "https://img.icons8.com/color/96/button.png",
    ),
    MaterialComponentModel(
        4,
        "Cards",
        "Contain content and actions about a single subject",
        NavKey::Cards,
        "https://img.icons8.com/color/96/bank-card-back-side.png",
    ),
    MaterialComponentModel(
        5,
        "Carousel",
        "A horizontally scrollable list of items",
        NavKey::Carousel,
        "https://img.icons8.com/color/96/image-gallery.png",
    ),
    MaterialComponentModel(
        6,
        "CheckBox",
        "Allows users to select one or more items",
        NavKey::CheckBox,
        "https://img.icons8.com/color/96/checked-checkbox.png",
    ),
    MaterialComponentModel(
        7,
        "Chip",
        "Compact elements that represent an attribute, text, entity, or action",
        NavKey::Chip,
        "https://img.icons8.com/color/96/tag-window.png",
    ),
    MaterialComponentModel(
        8,
        "Column",
        "A layout composable that places its children in a vertical sequence",
        NavKey::ColumnScreen,
        "https://img.icons8.com/color/96/vertical-settings-mixer.png",
    ),
    MaterialComponentModel(
        9,
        "DatePicker",
        "Allows users to select a date",
        NavKey::DatePicker,
        "https://img.icons8.com/color/96/calendar--v1.png",
    ),
    MaterialComponentModel(
        10,
        "Dialog",
        "Inform users about a task and can contain critical information",
        NavKey::Dialog,
        "https://img.icons8.com/color/96/chat.png",
    ),
    MaterialComponentModel(
        11,
        "FullScreenDialog",
        "A dialog that covers the entire screen",
        NavKey::FullScreenDialog,
        "https://img.icons8.com/color/96/full-screen.png",
    ),
    MaterialComponentModel(
        12,
        "Elevated Button",
        "A button with a shadow that increases when pressed",
        NavKey::ElevatedButton,
        "https://img.icons8.com/color/96/button.png",
    ),
    MaterialComponentModel(
        13,
        "Filled Tonal Button",
        "A button with a secondary color background",
        NavKey::FilledTonalButton,
        "https://img.icons8.com/color/96/button.png",
    ),
    MaterialComponentModel(
        14,
        "Icon Button",
        "A button that displays an icon",
        NavKey::IconButton,
        "https://img.icons8.com/color/96/circled-menu.png",
    ),
    MaterialComponentModel(
        15,
        "LazyColumn",
        "A vertically scrolling list that only composes visible items",
        NavKey::LazyColumnScreen,
        "https://img.icons8.com/color/96/list.png",
    ),
    MaterialComponentModel(
        16,
        "LazyRow",
        "A horizontally scrolling list that only composes visible items",
        NavKey::LazyRowScreen,
        "https://img.icons8.com/color/96/horizontal-settings-mixer.png",
    ),
    MaterialComponentModel(
        17,
        "Menu",
        "Displays a list of choices on a temporary surface",
        NavKey::Menu,
        "https://img.icons8.com/color/96/menu.png",
    ),
    MaterialComponentModel(
        18,
        "Bottom Navigation Bar",
        "Provides quick access to primary destinations",
        NavKey::BottomNavigationBar,
        "https://img.icons8.com/color/96/menu.png",
    ),
    MaterialComponentModel(
        19,
        "Navigation Drawer",
        "Provides access to destinations in your app",
        NavKey::NavigationDrawer,
        "https://img.icons8.com/color/96/menu.png",
    ),
    MaterialComponentModel(
        20,
        "Outline Button",
        "A button with a border and no background",
        NavKey::OutlineButton,
        "https://img.icons8.com/color/96/button.png",
    ),
    MaterialComponentModel(
        21,
        "Progress Indicator",
        "Indicates the status of an ongoing process",
        NavKey::ProgressIndicator,
        "https://img.icons8.com/color/96/loading.png",
    ),
    MaterialComponentModel(
        22,
        "Radio",
        "Allows users to select one option from a set",
        NavKey::Radio,
        "https://img.icons8.com/color/96/radio.png",
    ),
    MaterialComponentModel(
        23,
        "Row",
        "A layout composable that places its children in a horizontal sequence",
        NavKey::RowScreen,
        "https://img.icons8.com/color/96/horizontal-settings-mixer.png",
    ),
    MaterialComponentModel(
        24,
        "Scaffold",
        "Implements the basic Material Design visual layout structure",
        NavKey::ScaffoldScreen,
        "https://img.icons8.com/color/96/template.png",
    ),
    MaterialComponentModel(
        25,
        "Multi Choice Segment Button",
        "Allows selecting multiple segments",
        NavKey::MultiChoiceSegmentButton,
        "https://img.icons8.com/color/96/toggle-on.png",
    ),
    MaterialComponentModel(
        26,
        "Single Choice Segmented Button",
        "Allows selecting a single segment",
        NavKey::SingleChoiceSegmentedButton,
        "https://img.icons8.com/color/96/toggle-off.png",
    ),
    MaterialComponentModel(
        27,
        "Slider",
        "Allows users to make selections from a range of values",
        NavKey::Slider,
        "https://img.icons8.com/color/96/toggle-off.png",
    ),
    MaterialComponentModel(
        28,
        "SnackBar",
        "Provides brief messages about app processes at the bottom",
        NavKey::SnackBar,
        "https://img.icons8.com/color/96/appointment-reminders.png",
    ),
    MaterialComponentModel(
        29,
        "Spacer",
        "Represents an empty space with flexible size",
        NavKey::SpacerScreen,
        "https://img.icons8.com/color/96/resize-horizontal.png",
    ),
    MaterialComponentModel(
        30,
        "Switch",
        "Allows users to toggle between two states",
        NavKey::Switch,
        "https://img.icons8.com/color/96/toggle-on.png",
    ),
    MaterialComponentModel(
        31,
        "Tabs",
        "Organize content across different screens and views",
        NavKey::Tabs,
        "https://img.icons8.com/color/96/toggle-on.png",
    ),
    MaterialComponentModel(
        32,
        "Text Button",
        "A button with no border and no background",
        NavKey::TextButton,
        "https://img.icons8.com/color/96/button.png",
    ),
    MaterialComponentModel(
        33,
        "TextField",
        "Allows users to enter and edit text",
        NavKey::TextField,
        "https://img.icons8.com/color/96/edit-property.png",
    ),
    MaterialComponentModel(
        34,
        "TimePicker",
        "Allows users to select a time",
        NavKey::TimePicker,
        "https://img.icons8.com/color/96/clock--v1.png",
    ),
    MaterialComponentModel(
        35,
        "Toolbar",
        "A standard app bar used at the top of screens",
        NavKey::Toolbar,
        "https://img.icons8.com/color/96/clock--v1.png",
    ),
    MaterialComponentModel(
        36,
        "ToolTips",
        "Provide informative text when users hover over or long-press",
        NavKey::ToolTips,
        "https://img.icons8.com/color/96/info.png",
    ),
    MaterialComponentModel(
        37,
        "Box",
        "A layout composable that stacks its children on top of each other",
        NavKey::BoxScreen,
        "https://img.icons8.com/color/96/square.png",
    ),
    MaterialComponentModel(
        38,
        "TopAppBar",
        "Displays information and actions at the top of a screen",
        NavKey::TopAppBarScreen,
        "https://img.icons8.com/color/96/top-menu.png",
    ),
    MaterialComponentModel(
        39,
        "State (ViewModel)",
        "Study how to manege state in Jetpack compose",
        NavKey::StateViewModelScreen,
        "https://img.icons8.com/color/96/top-menu.png",
    ),
    MaterialComponentModel(
        40,
        "Network",
        "Detail about request api in android",
        NavKey::UserApiScreen,
        "https://img.icons8.com/color/96/wifi.png",
    ),
    MaterialComponentModel(
        41,
        "Room Database",
        "Detail about Room Database in Android",
        NavKey::TaskScreen,
        "https://img.icons8.com/color/96/database.png",
    ),
    MaterialComponentModel(
        42,
        "Pick Single Photo",
        "Select a single photo from the device using the Android Photo Picker.",
        NavKey::PickerSinglePhotoScreen,
        "https://img.icons8.com/color/96/image.png",
    ),
    MaterialComponentModel(
        43,
        "Pick Multiple Photos",
        "Select multiple photos from the device using the Android Photo Picker.",
        NavKey::PickerMultiPhotoScreen,
        "https://img.icons8.com/color/96/pictures-folder.png",
    ),
    MaterialComponentModel(
        44,
        "Pick Single Video",
        "Select a single video from the device using the Android Photo Picker.",
        NavKey::PickerSingleVideoScreen,
        "https://img.icons8.com/color/96/video.png",
    ),
    MaterialComponentModel(
        45,
        "Pick Multiple Videos",
        "Select multiple videos from the device using the Android Photo Picker.",
        NavKey::PickerMultiVideoScreen,
        "https://img.icons8.com/color/96/video-playlist.png",
    ),
    MaterialComponentModel(
        46,
        "Pick Photos & Videos",
        "Select photos and videos together using the Android Photo Picker.",
        NavKey::PickerVideoAndPhotos,
        "https://img.icons8.com/color/96/gallery.png",
    ),
    MaterialComponentModel(
        47,
        "Open File",
        "Browse device storage and select a file using the Android file picker.",
        NavKey::PickFromFile,
        "https://img.icons8.com/color/96/opened-folder.png",
    ),
    MaterialComponentModel(
        48,
        "CameraX",
        "Open the camera and capture a photo using the Android CameraX library.",
        NavKey::CameraHardware,
        "https://img.icons8.com/color/96/camera.png",
    ),
    MaterialComponentModel(
        49,
        "Biometric",
        "Authenticate the user securely using fingerprint or biometric authentication.",
        NavKey::FingerPrint,
        "https://img.icons8.com/color/96/fingerprint.png",
    ),
    MaterialComponentModel(
        50,
        "Location",
        "Access the user's current location using Android location services and runtime permissions.",
        NavKey::GetLocation,
        "https://img.icons8.com/color/96/marker.png",
    ),
    MaterialComponentModel(
        51,
        "Battery",
        "Monitor the device battery percentage and charging status using Android BroadcastReceiver.",
        NavKey::Battery,
        "https://img.icons8.com/color/96/battery.png",
    ),
)